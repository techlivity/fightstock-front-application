...
import {columns, categories, statusOptions} from "./data";
...
export default function RenderCategories() {
  ...
  const renderCell = React.useCallback((categorie, columnKey) => {
    ...
    switch (columnKey) {
      case "name":
        return (
          <User
            ...
            avatarProps={{radius: "lg", src: categorie.image}}
      ...
      case "status":
        return (
          <Chip className="capitalize" color={statusColorMap[categorie.status]} size="sm" variant="flat">
            {cellValue}
          </Chip>
        );
      case "actions":
